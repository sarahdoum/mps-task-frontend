  ...
  return (
    <div className='w-full h-32 bg-white p-5 shadow-md rounded-md flex items-center justify-between'>
      <div className='h-full flex flex-1 flex-col justify-between'>
        ...
        <span className='text-sm text-gray-400'>{"Le mois dernier"}</span>
      </div>
      <div
        className={clsx(
  ...
  const TableRow = ({ user }) => (
    <tr className='border-b border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-400/10'>
      ...
      <td>
        ...
        >
          {user?.isActive ? "Active" : "Désactivé"}
        </p>
      </td>
...
const TaskTable = ({ tasks }) => {
  ...
  const TableHeader = () => (
    <thead className='border-b border-gray-300 dark:border-gray-600'>
      <tr className='text-black dark:text-white  text-left'>
        ...
        <th className='py-2 hidden md:block'>Créé</th>
      </tr>
    </thead>
  );
  ...
  return (
    <>
      <div
        className={clsx(
          "w-full bg-white dark:bg-[#1f1f1f] px-2 md:px-4 pt-4 pb-4 shadow-md rounded-lg",
      ...
      >
        <table className='w-full '>
          <TableHeader />
          <tbody className=''>
            {tasks.slice(0, 9).map((task, id) => ( // Modification ici
